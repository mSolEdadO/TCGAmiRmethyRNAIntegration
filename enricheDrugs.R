...
#data=lapply(data,function(x) x%>%filter(padj<0.05))
data=do.call(rbind,lapply(1:length(files),function(x) cbind(gsub(".gsea","",files[x]),data[[x]])))
data$cell=sapply(strsplit(data[,1],".",fixed=T),function(x) x[2])
data$iteration=sapply(strsplit(data[,1],".",fixed=T),function(x) x[1])

meta <- loadCMapData("../../cmapMetadata.txt",type="metadata")
ids=meta[,c("sig_id","pert_iname")]
drugs <- loadCMapData("../../cmapCompoundInfo.txt", type="compoundInfo")

data=merge(data,ids,by="sig_id")
data=merge(data,drugs,by="pert_iname")
launched=data%>%filter(clinical_phase=="Launched")
